...
class TestMatrixDeterminant:
    ...
    def test_determinant_1x1(self):
        m = Matrix([[7]])
        print(f"\n✓ det([[7]]) = {determinant(m)} (expected 7)")
        assert determinant(m) == 7

    def test_determinant_1x1_negative(self):
        m = Matrix([[-3]])
        print(f"\n✓ det([[-3]]) = {determinant(m)} (expected -3)")
        assert determinant(m) == -3

    def test_determinant_2x2(self):
        m = Matrix([[1, 2], [3, 4]])
        print(f"\n✓ det([[1,2],[3,4]]) = {determinant(m)} (expected -2)")
        assert determinant(m) == -2

    def test_determinant_2x2_positive(self):
        m = Matrix([[3, 1], [2, 4]])
        print(f"\n✓ det([[3,1],[2,4]]) = {determinant(m)} (expected 10)")
        assert determinant(m) == 10
    ...
    def test_determinant_2x2_identity(self):
        m = Matrix([[1, 0], [0, 1]])
        print(f"\n✓ det(I_2) = {determinant(m)} (expected 1)")
        assert determinant(m) == 1

    def test_determinant_3x3(self):
        m = Matrix([[1, 2, 3], [4, 5, 6], [7, 8, 9]])
        print(f"\n✓ det(3x3 singular) = {determinant(m)} (expected 0)")
        assert determinant(m) == 0

    def test_determinant_3x3_nonzero(self):
        m = Matrix([[1, 2, 0], [3, 4, 5], [6, 0, 7]])
        print(f"\n✓ det([[1,2,0],[3,4,5],[6,0,7]]) = {determinant(m)} (expected 46)")
        assert determinant(m) == 46

    def test_determinant_3x3_identity(self):
        m = Matrix([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
        print(f"\n✓ det(I_3) = {determinant(m)} (expected 1)")
        assert determinant(m) == 1
    ...
    def test_determinant_4x4(self):
        ...
        print(f"\n✓ det(4x4 singular) = {determinant(m)} (expected 0)")
        assert determinant(m) == 0
    ...
    def test_determinant_4x4_nonzero(self):
        ...
        print(f"\n✓ det(4x4 tridiagonal) = {determinant(m)} (expected 85)")
        assert determinant(m) == 85

    def test_determinant_upper_triangular(self):
        m = Matrix([[2, 3, 5], [0, 4, 7], [0, 0, 6]])
        print(
            f"\n✓ det(upper triangular) = {determinant(m)} (expected 48, product of diagonal)"
        )
        assert determinant(m) == 48

    def test_determinant_lower_triangular(self):
        m = Matrix([[3, 0, 0], [5, 2, 0], [1, 4, 6]])
        print(
            f"\n✓ det(lower triangular) = {determinant(m)} (expected 36, product of diagonal)"
        )
        assert determinant(m) == 36

    def test_determinant_with_zeros_on_first_row(self):
        m = Matrix([[0, 1, 2], [3, 4, 5], [6, 7, 8]])
        print(f"\n✓ det(first row starts with zeros) = {determinant(m)} (expected 0)")
        assert determinant(m) == 0

    def test_determinant_with_all_zeros_first_row(self):
        m = Matrix([[0, 0, 0], [1, 2, 3], [4, 5, 6]])
        print(f"\n✓ det(all-zero first row) = {determinant(m)} (expected 0)")
        assert determinant(m) == 0
    ...
    def test_determinant_scalar_multiple(self):
        m = Matrix([[2, 4], [6, 8]])
        print(f"\n✓ det([[2,4],[6,8]]) = {determinant(m)} (expected -8)")
        assert determinant(m) == -8
    ...
    def test_determinant_float_entries(self):
        ...
        print(f"\n✓ det([[1.5,2.0],[3.0,4.5]]) = {determinant(m)} (expected 0.75)")
        assert abs(determinant(m) - 0.75) < 1e-10

    def test_determinant_matches_ad_minus_bc(self):
        m = Matrix([[5, 7], [2, 3]])
        expected = (5 * 3) - (7 * 2)
        print(f"\n✓ det([[5,7],[2,3]]) = {determinant(m)} (expected {expected})")
